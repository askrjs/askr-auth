...
export async function createRequest(
  ...
  root.setAttribute("xmlns:samlp", NS.protocol);
  root.setAttribute("xmlns:saml", NS.assertion);
  root.setAttribute("ID", requestId);
  root.setAttribute("Version", "2.0");
  root.setAttribute("IssueInstant", new Date(now).toISOString());
  root.setAttribute("Destination", options.idp.ssoUrl);
  ...
  const issuer = doc.createElementNS(NS.assertion, "saml:Issuer");
  issuer.appendChild(doc.createTextNode(options.entityId));
