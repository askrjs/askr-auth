...
function base64url(value: string): Uint8Array {
  if (!/^[A-Za-z0-9_-]*$/u.test(value) || value.length % 4 === 1)
    fail("malformed-input", "Client data contains invalid base64url.");
  let binary: string;
  try {
    binary = atob(
      value
    ...
    return fail("malformed-input", "Client data contains invalid base64url.");
  }
...
function clientData(
  ...
  try {
    ...
    fail("malformed-input", "Client data JSON is invalid.");
  }
  if (!value || typeof value !== "object" || Array.isArray(value))
    fail("malformed-input", "Client data must be an object.");
  const data = value as Record<string, unknown>;
  if (data.type !== type) fail("malformed-input", "Client data type is invalid.");
  if (typeof data.challenge !== "string" || !equal(base64url(data.challenge), challenge))
    fail("invalid-challenge", "WebAuthn challenge does not match.");
  if (typeof data.origin !== "string" || !origins.includes(data.origin))
    fail("invalid-origin", "WebAuthn origin is not allowed.");
  if (data.crossOrigin === true || (data.crossOrigin !== undefined && data.crossOrigin !== false))
    fail("invalid-origin", "Cross-origin WebAuthn ceremonies are not allowed.");
}
interface AuthData {
  flags: number;
  signCount: number;
...
async function parseAuthData(
  ...
  if (!equal(input.subarray(0, 32), await sha256(rpId)))
    fail("invalid-rp-id", "RP ID hash does not match.");
  ...
  if (backedUp && !backupEligible)
    fail("malformed-input", "Backup state requires backup eligibility.");
  const signCount = new DataView(input.buffer, input.byteOffset + 33, 4).getUint32(0);
  let offset = 37;
  const result: AuthData = { flags, signCount, backupEligible, backedUp };
  if (registration) {
    if (!(flags & 64) || input.length < 55)
      fail("malformed-input", "Attested credential data is required.");
    result.aaguid = input.slice(offset, offset + 16);
    offset += 16;
    const length = new DataView(input.buffer, input.byteOffset + offset, 2).getUint16(0);
    offset += 2;
    if (!length || offset + length > input.length)
      fail("malformed-input", "Credential ID is invalid.");
    result.credentialId = input.slice(offset, offset + length);
    offset += length;
    const decoded = decodeCborFirst(input.subarray(offset));
    result.cose = input.slice(offset, offset + decoded.bytesRead);
    offset += decoded.bytesRead;
  } else if (flags & 64)
    fail("malformed-input", "Authentication data unexpectedly contains attested credential data.");
  if (flags & 128) {
    const extension = decodeCborFirst(input.subarray(offset));
    offset += extension.bytesRead;
  }
  if (offset !== input.length)
    fail("malformed-input", "Authenticator data contains trailing bytes.");
  return result;
}
function algorithmFor(key: JsonWebKey): {
  import: AlgorithmIdentifier | EcKeyImportParams | RsaHashedImportParams;
...
function derEcdsa(signature: Uint8Array): Uint8Array {
  ...
  if (
    ...
    fail("malformed-input", "ECDSA signature is not strict DER.");
  const rLength = signature[3];
  const rStart = 4;
  const sMarker = rStart + rLength;
  if (rLength < 1 || sMarker + 2 > signature.length || signature[sMarker] !== 0x02)
    fail("malformed-input", "ECDSA signature is not strict DER.");
  const sLength = signature[sMarker + 1];
  const sStart = sMarker + 2;
  if (sLength < 1 || sStart + sLength !== signature.length)
    fail("malformed-input", "ECDSA signature is not strict DER.");
  const integer = (start: number, length: number) => {
    const part = signature.subarray(start, start + length);
    if (part[0] & 0x80 || (part.length > 1 && part[0] === 0 && !(part[1] & 0x80)))
      fail("malformed-input", "ECDSA signature integer is not minimally encoded.");
    ...
    output.set(unsigned, 32 - unsigned.length);
    return output;
  };
  const output = new Uint8Array(64);
  output.set(integer(rStart, rLength));
  output.set(integer(sStart, sLength), 32);
  return output;
}
...
export async function verifyWebAuthnRegistration(
  ...
  if (
    !(decodedObject instanceof Map) ||
    typeof decodedObject.get("fmt") !== "string" ||
    !(decodedObject.get("authData") instanceof Uint8Array) ||
    !(decodedObject.get("attStmt") instanceof Map)
  )
    fail("malformed-input", "Attestation object is invalid.");
  const object = decodedObject as Map<unknown, unknown>;
  const authBytes = object.get("authData") as Uint8Array;
  const auth = await parseAuthData(
    authBytes,
    input.rpId,
    input.requireUserVerification ?? true,
    true,
  );
  if (!equal(auth.credentialId!, input.credentialId))
    fail("credential-mismatch", "Credential ID does not match attested data.");
  const decoded = decodeCosePublicKey(auth.cose!);
  const statement = object.get("attStmt") as Map<unknown, unknown>;
  const format = object.get("fmt");
  if (format === "none") {
    if (statement.size)
      fail("invalid-attestation", "None attestation must have an empty statement.");
  } else if (format === "packed") {
    if (
      statement.has("x5c") ||
      statement.get("alg") !== decoded.algorithm ||
      !(statement.get("sig") instanceof Uint8Array)
    )
      fail("invalid-attestation", "Only packed self-attestation is accepted.");
    const signed = new Uint8Array(authBytes.length + 32);
    signed.set(authBytes);
    signed.set(await sha256(input.clientDataJSON), authBytes.length);
    if (!(await verify(decoded.publicKeyJwk, statement.get("sig") as Uint8Array, signed)))
      fail("invalid-attestation", "Packed self-attestation signature is invalid.");
  } else fail("invalid-attestation", "Attestation format is unsupported.");
  return {
    credentialId: auth.credentialId!,
    publicKeyJwk: decoded.publicKeyJwk,
...
export async function verifyWebAuthnAuthentication(
  ...
  if (!(await verify(input.publicKeyJwk, input.signature, signed)))
    fail("invalid-signature", "Assertion signature is invalid.");
  return {
    signCount: auth.signCount,
    backupEligible: auth.backupEligible,
    backedUp: auth.backedUp,
