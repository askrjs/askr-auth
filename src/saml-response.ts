...
function instant(value: string | null, label: string): number {
  if (!value || !/^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d(?:\.\d+)?Z$/u.test(value))
...
function validateTime(element: XmlElement, now: number, skew: number, maxAge: number): void {
  ...
  if (issue > now + skew || issue < now - maxAge - skew)
    invalid("Assertion IssueInstant is outside the allowed window");
...
function extract(
  ...
  now: number,
): { principal: SamlPrincipal; requestId: string } {
  const doc = parseXml(assertionXml);
  checkUniqueIds(doc);
  const assertion = doc.documentElement;
  if (assertion.namespaceURI !== NS.assertion || assertion.localName !== "Assertion")
    malformed("Signed reference is not an assertion");
  if (assertion.getAttribute("Version") !== "2.0" || !assertion.getAttribute("ID"))
    malformed("Assertion version or ID is invalid");
  const issuer = textOf(one(assertion, NS.assertion, "Issuer"));
  if (issuer !== options.idp.entityId) invalid("Unexpected assertion issuer");
  validateTime(
    assertion,
    now,
    (options.clockSkewSeconds ?? 60) * 1000,
    (options.maxAssertionAgeSeconds ?? 300) * 1000,
  );
  ...
  if (
    ...
    !restrictions.every((r) =>
      children(r, NS.assertion, "Audience").some((a) => textOf(a) === options.entityId),
    )
  )
    invalid("Unexpected audience");
  const subject = one(assertion, NS.assertion, "Subject");
  const nameId = one(subject, NS.assertion, "NameID");
  const confirmations = children(subject, NS.assertion, "SubjectConfirmation").filter(
    (node) => node.getAttribute("Method") === "urn:oasis:names:tc:SAML:2.0:cm:bearer",
  );
  ...
  const requestId = data.getAttribute("InResponseTo");
  if (!requestId) invalid("Missing signed InResponseTo");
  const expires = instant(
    data.getAttribute("NotOnOrAfter"),
    "SubjectConfirmationData NotOnOrAfter",
  );
  if (now - (options.clockSkewSeconds ?? 60) * 1000 >= expires)
    invalid("Subject confirmation has expired");
  ...
  if (authnInstant > now + (options.clockSkewSeconds ?? 60) * 1000)
    invalid("Authentication occurred in the future");
  const attributes: Record<string, string[]> = {};
  for (const statement of children(assertion, NS.assertion, "AttributeStatement"))
    for (const attribute of children(statement, NS.assertion, "Attribute")) {
      const name = attribute.getAttribute("Name");
      if (!name || Object.hasOwn(attributes, name)) invalid("Invalid or duplicate attribute name");
      attributes[name] = children(attribute, NS.assertion, "AttributeValue").map(
  ...
  return {
    ...
    principal: {
      ...
      saml: {
        issuer,
        nameId: id,
        ...(nameId.getAttribute("Format") ? { nameIdFormat: nameId.getAttribute("Format")! } : {}),
        ...(statements[0].getAttribute("SessionIndex")
          ? { sessionIndex: statements[0].getAttribute("SessionIndex")! }
          : {}),
        attributes,
      },
    },
...
export async function validate(
  input: { samlResponse: string; relayState?: string },
  options: SamlServiceProviderOptions,
): Promise<SamlPrincipal> {
  const responseXml = decode(input.samlResponse);
  const doc = parseXml(responseXml);
  checkUniqueIds(doc);
  let response = doc.documentElement;
  if (response.namespaceURI !== NS.protocol || response.localName !== "Response")
    malformed("Expected one SAML Response root");
  if (response.getAttribute("Version") !== "2.0" || !response.getAttribute("ID"))
    malformed("Response version or ID is invalid");
  instant(response.getAttribute("IssueInstant"), "Response IssueInstant");
  if (children(response, NS.protocol, "Status").length !== 1)
    malformed("Expected one response Status");
  const status = one(one(response, NS.protocol, "Status"), NS.protocol, "StatusCode").getAttribute(
    "Value",
  );
  if (status !== "urn:oasis:names:tc:SAML:2.0:status:Success")
    throw new SamlValidationError("idp-error", "Identity provider returned an error");
  ...
  if (options.requireSignedResponse && !responseSigned)
    throw new SamlValidationError("invalid-signature", "Signed response required");
  if (responseSigned) {
    const verifiedResponse = parseXml(
      verifySignedElement(responseXml, response, options.idp.certificates),
    );
    checkUniqueIds(verifiedResponse);
    response = verifiedResponse.documentElement;
  }
  ...
  if (originalSignatureCount > maximumSignatures)
    malformed("Unexpected signature placement or count");
  let assertionXml: string;
  if (encrypted.length) {
    if (!options.decryptAssertions)
      throw new SamlValidationError("invalid-signature", "Encrypted assertion is not configured");
  ...
  const assertionDoc = parseXml(assertionXml);
  checkUniqueIds(assertionDoc);
  ...
  if (responseSigned) {
    ...
    const correlation = response.getAttribute("InResponseTo");
    if (destination !== options.acsUrl || correlation !== result.requestId)
      invalid("Signed response correlation mismatch");
  }
  const stored = await options.requestStore.get(result.requestId);
  if (!stored || stored.expiresAt <= (options.clock ?? Date.now)())
    throw new SamlValidationError("unknown-request", "Unknown or expired SAML request");
  if (stored.relayState !== input.relayState) invalid("RelayState mismatch");
  if (!(await options.requestStore.consume(result.requestId)))
    throw new SamlValidationError("replayed-response", "SAML response has already been consumed");
...
function invalid(message: string): never {
  throw new SamlValidationError("invalid-claim", message);
}
