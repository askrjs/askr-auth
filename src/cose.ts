...
export function decodeCosePublicKey(input: Uint8Array): DecodedCosePublicKey {
  ...
  if (!(value instanceof Map))
    throw new MfaValidationError("malformed-input", "COSE key must be a map.");
  ...
  if (algorithm === -7 && kty === 2 && value.get(-1) === 1) {
    return {
      ...
      publicKeyJwk: {
        kty: "EC",
        crv: "P-256",
        x: b64(bytes(value.get(-2), 32)),
        y: b64(bytes(value.get(-3), 32)),
        alg: "ES256",
        ext: true,
      },
    };
  ...
  if (algorithm === -257 && kty === 3) {
    ...
    if (n.length < 256 || e.length < 3)
      throw new MfaValidationError(
        "unsupported-algorithm",
        "RS256 keys must use RSA with at least 2048 bits.",
      );
    return {
      algorithm,
      publicKeyJwk: { kty: "RSA", n: b64(n), e: b64(e), alg: "RS256", ext: true },
    };
  }
