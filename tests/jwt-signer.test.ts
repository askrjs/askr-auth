...
async function keyPair(kind: "RS256" | "ES256") {
  ...
  return {
    ...
    verification:
      kind === "RS256" ? { name: "RSASSA-PKCS1-v1_5" } : { name: "ECDSA", hash: "SHA-256" },
  };
}
...
describe("JWT signer", () => {
  ...
  it.each(["alg", "kid", "crit", "b64"])("should reject protected header %s", async (name) => {
    ...
    await expect(signer.sign({ claims: {}, protectedHeader: { [name]: "value" } })).rejects.toThrow(
      TypeError,
    );
  });
  ...
  it.each(["iss", "sub", "aud", "iat", "exp", "jti"])(
    "should reject timed claim collision %s",
    async (name) => {
      const pair = await keyPair("ES256");
      const signer = createJwtSigner({ privateKey: pair.privateKey, kid: "key" });
      await expect(
