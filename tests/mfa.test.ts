...
function cbor(value: unknown): Uint8Array {
  ...
  if (typeof value === "string") {
    ...
    return join(prefix(3, encoded.length), encoded);
  }
  if (value instanceof Uint8Array) return join(prefix(2, value.length), value);
  if (value instanceof Map)
    return join(
      prefix(5, value.size),
      ...Array.from(value).flatMap(([key, item]) => [cbor(key), cbor(item)]),
    );
...
describe("TOTP", () => {
  it.each([
    ...
    "should give a valid RFC 6238 code when %s is selected",
    async (algorithm, secret, code) => {
      await expect(
        verifyTotpCode({ secret, code, algorithm, digits: 8, at: new Date(59_000), window: 0 }),
  ...
  it("should give the matching counter and drift when a neighboring step is valid", async () => {
    ...
    ).resolves.toEqual({ valid: true, counter: 1, drift: -1 });
  });
  ...
  it("should give an encoded otpauth URI when issuer and account contain reserved characters", () => {
    ...
    expect(uri).toBe(
      "otpauth://totp/Acme%20%26%20Co:a%2Bb%40example.com?secret=GEZDGNBVGY3TQOJQ&issuer=Acme+%26+Co&algorithm=SHA1&digits=6&period=30",
    );
  });
...
describe("CBOR", () => {
  it("should give the decoded subset when a bounded value is valid", () => {
    expect(
      decodeCbor(Uint8Array.of(0xa2, 0x01, 0x42, 0xaa, 0xbb, 0x61, 0x61, 0x83, 0x01, 0xf5, 0xf6)),
    ).toEqual(
      new Map([
        [1, Uint8Array.of(0xaa, 0xbb)],
        ["a", [1, true, null]],
      ]),
    );
  });
...
describe("WebAuthn", () => {
  it.each([
    ...
    "should reject an oversized registration %s before parsing",
    async (_label, field, size) => {
      await expect(
        verifyWebAuthnRegistration({
  ...
  it.each([
    ...
    "should reject oversized authentication %s before verification",
    async (_label, field, size) => {
      await expect(
        verifyWebAuthnAuthentication({
          credentialId: Uint8Array.of(1),
  ...
  it.each([
    ...
    { storedSignCount: 1, assertionSignCount: 2 },
  ])(
    "should give $assertionSignCount when an ES256 assertion follows stored counter $storedSignCount",
    async ({ storedSignCount, assertionSignCount }) => {
      const pair = await crypto.subtle.generateKey({ name: "ECDSA", namedCurve: "P-256" }, true, [
        "sign",
        "verify",
      ]);
      ...
      new DataView(authenticatorData.buffer).setUint32(33, assertionSignCount);
      const clientDataJSON = bytes(
