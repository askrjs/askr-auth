...
describe("WebAuthn client", () => {
  it("should round-trip canonical base64url", () => {
    expect(new Uint8Array(decodeBase64Url(encoded(0, 255, 126)))).toEqual(
      new Uint8Array([0, 255, 126]),
    );
  });
  ...
  it("should create a discoverable passkey with secure defaults and server-aligned output", async () => {
    ...
    await expect(
      ...
      credentialId: encoded(1, 2),
      clientDataJSON: encoded(3),
      attestationObject: encoded(4),
    });
  ...
  it.each([undefined, []] as const)(
    "should omit allowCredentials for discoverable authentication",
    async (allowCredentials) => {
      ...
      await expect(
        getPasskeyAssertion({ challenge: encoded(9), rpId: "example.test", allowCredentials }),
      ).resolves.toEqual({
        credentialId: encoded(1),
        clientDataJSON: encoded(2),
        authenticatorData: encoded(3),
        signature: encoded(4),
      });
  ...
  it("should reject unavailable APIs and invalid credential shapes with TypeError", async () => {
    ...
    await expect(
      getPasskeyAssertion({ challenge: encoded(9), rpId: "example.test" }),
