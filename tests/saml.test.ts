...
class Store {
  ...
  async save(request: SamlStoredRequest) {
...
function signedAssertion(
  requestId: string,
  overrides: { audience?: string; recipient?: string; issuer?: string } = {},
): string {
...
describe("SAML service provider", () => {
  ...
  it("should give a valid RSA-SHA256 signature when request signing is configured", async () => {
    ...
    const service = createSamlServiceProvider({
      ...
      requestStore: store,
      clock: () => now,
      signRequests: { privateKey, certificate },
    });
    ...
    expect(
      verify("RSA-SHA256", Buffer.from(canonical), certificate, Buffer.from(signature, "base64")),
    ).toBe(true);
  });
  ...
  it("should give structured claims when a signed assertion is valid", async () => {
    ...
    await expect(
      service.validateResponse({
        samlResponse: response(signedAssertion(requestId), requestId),
        relayState: "state",
      }),
      ...
      saml: {
        ...
        attributes: { groups: ["admin", "billing"] },
      },
    });
  });
  ...
  it("should give a replay error when a response is validated twice", async () => {
    ...
    await expect(
      service.validateResponse({ samlResponse: encoded, relayState: "state" }),
    ).rejects.toMatchObject({ code: "replayed-response" });
  });
  ...
  it("should give one success when responses consume a request concurrently", async () => {
    ...
    const settled = await Promise.allSettled([
      service.validateResponse({ samlResponse: encoded }),
      service.validateResponse({ samlResponse: encoded }),
    ]);
  ...
  it("should give a principal when an RSA-OAEP and AES-GCM assertion is valid", async () => {
    ...
    const service = createSamlServiceProvider({
      ...
      requestStore: store,
      clock: () => now,
      decryptAssertions: { privateKey, certificate },
    });
    const { requestId } = await service.createAuthnRequest();
    const encrypted = await encryptedAssertion(signedAssertion(requestId));
    await expect(
      service.validateResponse({ samlResponse: response(encrypted, requestId) }),
    ).resolves.toMatchObject({ id: "user@example.com" });
  });
  ...
  it("should give a principal when a required outer response signature is valid", async () => {
    ...
    await expect(
      service.validateResponse({
        samlResponse: signedResponse(signedAssertion(requestId), requestId),
      }),
    ).resolves.toMatchObject({ id: "user@example.com" });
  });
  ...
  it.each([
    ...
    ["audience", { audience: "https://evil.example" }],
    ["recipient", { recipient: "https://evil.example" }],
  ])(
    "should give an invalid-claim error when the verified %s is invalid",
    async (_label, overrides) => {
      const { service } = setup();
      const { requestId } = await service.createAuthnRequest();
      await expect(
        service.validateResponse({
          samlResponse: response(signedAssertion(requestId, overrides), requestId),
        }),
  ...
  it("should give a malformed-response error when multiple assertions are present", async () => {
    ...
    await expect(service.validateResponse({ samlResponse: wrapped })).rejects.toMatchObject({
  ...
  it("should give an invalid-signature error when an assertion is unsigned", async () => {
    ...
    await expect(
      service.validateResponse({
        samlResponse: response(signed.replace(/<Signature[\s\S]*<\/Signature>/u, ""), requestId),
      }),
